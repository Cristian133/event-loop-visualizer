...
@Component({
  ...
  template: `
    <div class="panel">
      <div class="header">
        <h3 class="panel-title">Ejemplo JS</h3>
        <select class="exercise-select" (change)="onSelect($event)">
          @for (ex of allExercises(); track ex.id) {
            <option [value]="ex.id" [selected]="ex.id === currentExercise().id">{{ ex.name }}</option>
          }
        </select>
      </div>
      
      <p class="description">{{ currentExercise().description }}</p>
      
      <div class="code-container">
        @for (line of codeLines(); track $index) {
          <div class="code-line" [class.active]="$index === activeLine()">
  ...
  styles: `
    ...
    .code-line.active::before {
      ...
      width: 4px;
      background: var(--info);
      box-shadow: 0 0 15px var(--info), 0 0 5px var(--info);
      z-index: 2;
    }
    ...
    @keyframes linePulse {
      0%, 100% { background-color: rgba(0, 229, 255, 0.1); }
      50% { background-color: rgba(0, 229, 255, 0.2); }
    }
    @keyframes arrowPulse {
      from { opacity: 0.4; transform: translateY(-50%) translateX(0); }
      to { opacity: 1; transform: translateY(-50%) translateX(3px); }
    }
    .line-text {
      white-space: pre;
    }
